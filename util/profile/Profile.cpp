...
void Profile::accept(ProfileVisitor* visitor) {
  visitor->visit(this);
}

void Profile::addChild(Profile* profile) {
  childs->push_back(profile);
}

void Profile::setStart(double theTime) {
  start = theTime;
  
}

void Profile::setEnd(double theTime) {
  end = theTime;
}

double Profile::getStart() {
  return(start);
}

double Profile::getEnd() {
  return(end);
}

void Profile::addToTotal(double v) {
  total += v;
}
...
bool Profile::equals(const std::string* _key) {
  return((key == (*_key) ));
}

ProfileIterator* Profile::iterator() {
