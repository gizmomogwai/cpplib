...
long Date::getAsLong() const {
  return getYear() * 400 * 24 * 60 * 60 + getMonth() * 31 * 24 * 60 * 60 +
         getDay() * 24 * 60 * 60 + getHours() * 60 * 60 + getMinutes() * 60 +
         getSeconds();
}

std::string Date::toString() const {
  std::ostringstream out;
  out << getYear() << "-" << getMonth() << "-" << getDay() << "-" << getHours()
      << "-" << getMinutes() << "-" << getSeconds();
  return out.str();
}

time_t Date::getTimeT() { return mktime(&fCurrentTime); }
