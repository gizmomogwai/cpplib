...
void SocketOutputStream::write(int b) throw (IOException) {
  ...
  void SocketOutputStream::write(DataBuffer* b, int offset, int length)
    throw (IOException) {
  ...
  #error "so nicht"
#endif
	throw(Exception("SocketOutoutStream::write(DataBuffer, int, int) - error while sending",
                  __FILE__, __LINE__));
      }
    }
