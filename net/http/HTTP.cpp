...
HTTP::HTTP(std::string _host, int _port) 
  : host(_host), port(_port), authLine(0), s(0), fromServer(0), toServer(0), responce(0) {
}
...
void HTTP::setFile(std::string _resName) {
  resName = _resName;
}
...
InputStream* HTTP::get() throw (Exception) {
  ...
  std::auto_ptr<std::string> value = std::auto_ptr<std::string>(responce->getValue("Content-Range"));
  if (value.get() != 0) {
    throw(Exception("HTTP::get - ContentRange not supported", __FILE__, __LINE__));
    return 0;
  }
