...
void Socket::shutdownInput() {
  if (shutdown(theSocket, SD_RECEIVE) == SOCKET_ERROR) {
    throw(Exception("Socket::~Socket end send not possible", __FILE__, __LINE__));
  }
  is = 0;
}
...
Socket::~Socket() {
  ...
  if (closesocket(theSocket) == SOCKET_ERROR) {
    throw(Exception("Socket::~Socket - could not close socket", __FILE__, __LINE__));    
  }
...
InputStream* Socket::getInputStream() throw (Exception) {
  if (is == NULL) {
    throw(Exception("Socket::getInputStream - InputStream already closed", __FILE__, __LINE__)); 
  }
  return(is);
}
...
OutputStream* Socket::getOutputStream() throw (Exception) {
  if (os == NULL) {
    throw(Exception("Socket::getOutputStream - OutputStream already closed", __FILE__, __LINE__)); 
  }
  return(os);
}
