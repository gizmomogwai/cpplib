...
Image* JasperImageCodec::read(InputStream* _in) throw (Exception) {
  assert(_in != 0);
...
int JasperImageCodec::jasperRead(jas_stream_obj_t* obj, char* buf, unsigned int cnt) {
  int res = 0;
  if (cnt != 0) {
...
int JasperImageCodec::jasperWrite(jas_stream_obj_t* jStream, const char* buf, unsigned int cnt) {
  return 0;
}
