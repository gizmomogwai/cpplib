...
File::File(std::string _pathName) : pathName(_pathName) { unifyName(); }

File::File(File* f) : pathName(f->pathName) { unifyName(); }

File::File(const File& f) : pathName(f.pathName) { unifyName(); }
...
void File::rename(File* target) throw(IOException) {
  ...
  if (res != 0) {
    std::ostringstream h;
    h << "File \"" << pathName << "\" konnte nicht in \"" << target->pathName
      << "\" umbenannt werden.";
    IOException e(h.str(), __FILE__, __LINE__);
    throw(e);
  }
...
Date* File::getModificationDate() {
  ...
  HANDLE fileHandle;
  fileHandle =
      ::CreateFile(TEXT(getPathName().c_str()), GENERIC_READ, FILE_SHARE_READ,
  ...
  BOOL res = ::GetFileTime(fileHandle, &creationFileTime, &lastAccessFileTime,
                           &lastWriteFileTime);
  ...
  return new Date(st.wYear, st.wMonth, st.wDay, st.wHour, st.wMinute,
                  st.wSecond);
#endif // WIN32STUFF
}
