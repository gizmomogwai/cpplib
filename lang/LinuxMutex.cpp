...
Mutex::~Mutex() {
  ...
  if (res != 0) {
    throw(Exception("LinuxMutex::~LinuxMutex() - mutex in deconstructor still locked", 
                    __FILE__, __LINE__));
  }
...
void Mutex::lock() {
  ...
  if (res != 0) {
    throw(Exception("LinuxMutex::lock - could not lock", 
                    __FILE__, __LINE__));
  }
...
void Mutex::unlock() {
  ...
  if (res != 0) {
    throw(Exception("LinuxMutex::unlock - could not unlock", 
                    __FILE__, __LINE__));
  }
}
